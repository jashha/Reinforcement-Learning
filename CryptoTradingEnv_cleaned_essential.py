...
'''
            ...
            if not self.use_seq_obs: # # Original.
                obs = self.get_state() # Original.
            else: # Original.
                obs = self.get_seq_state(window_size=self.seq_len) # Original.

            self.current_step += 1

            '''obs = self.rolling_scaler.transform(obs.reshape(1, -1))'''
            # logging.info(f"||| obs.shape: {obs.shape} ||| np.mean(obs): {np.mean(obs)} ||| np.std(obs): {np.std(obs)} ||| np.min(obs): {np.min(obs)} ||| np.max(obs): {np.max(obs)} |||") #  ||| obs.shape[2]: {obs.shape[2]}")
            return obs, np.array(reward, dtype=np.float32), np.array(done, dtype=np.bool_), dict() # max_buy_hold=max(self.btc_buy_hold, self.eth_buy_hold, self.sp500_buy_hold))
        else: 
            ...
            if self.current_step >= self.end_step - 1:
                ...
                self.current_step += 1
                return obs, np.array(reward, dtype=np.float32), np.array(done, dtype=np.bool_), dict() # max_buy_hold=max(self.btc_buy_hold, self.eth_buy_hold, self.sp500_buy_hold)) # dict()

            if self.use_2d:
                return (self.state, self.obs), np.array(0, dtype=np.float32), np.array(done, dtype=np.bool_), dict()
            else:
                ...
                return obs, np.array(0., dtype=np.float32), np.array(done, dtype=np.bool_), dict() # max_buy_hold=max(self.btc_buy_hold, self.eth_buy_hold, self.sp500_buy_hold))  # dict()
