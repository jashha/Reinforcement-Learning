...
class PPOBuffer:
    ...
    """
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95, is_hyper_tune=False):
        self.obs_buf = np.zeros(combined_shape(size, obs_dim), dtype=np.float32)
        self.act_buf = np.zeros(combined_shape(size, act_dim), dtype=np.float32)
        '''self.act_buf = np.zeros(size, dtype=np.float32)'''
        # self.act_buf = np.zeros(combined_shape(size, (act_dim, act_dim)), dtype=np.float32)
    ...
    def store(self, obs, act, rew, val, logp):
        ...
        """
        assert self.ptr < self.max_size  # buffer has to have room to store
        self.obs_buf[self.ptr] = obs
        self.act_buf[self.ptr] = act # .item()
    ...
    def finish_path_(self, last_val=0):
        """
        ...
        # print(
        #     f"Finished path from {self.path_start_idx} to {self.ptr}"
        #     )
        # print(
        #     f"Rewards: {self.rew_buf[path_slice]}"
        #     )
        # print(
        #     f"Advantages: {self.adv_buf[path_slice]}"
        #     )
    ...
    def get(self):
        ...
        """
        assert self.ptr == self.max_size, "Buffer is not full!"    # buffer has to be full before you can get
        ...
        if not self.is_hyper_tune:
            ...
            self.adv_buf = (self.adv_buf - adv_mean) / adv_std
        print(f"Retrieving data with normalized advantages. Mean: {adv_mean}, Std: {adv_std}")

        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
                    adv=self.adv_buf, logp=self.logp_buf, val=self.val_buf)
        return {k: v if k=='obs' or k=='val' or k=='ret' else torch.as_tensor(v, dtype=torch.float32).to(device) for k,v in data.items()}
        # return {k: torch.as_tensor(v, dtype=torch.float32).to(device, non_blocking=True) for k,v in data.items()}
